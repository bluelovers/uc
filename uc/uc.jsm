...
for (let f in this)
	if (~f.lastIndexOf('UC_', 0)) UC[f.slice(3)] = this[f]

UC.lazy(function file2url() UC_file2url)
UC_lazy.call(this, function UC_file2url() Services.io.getProtocolHandler('file')
	.QueryInterface(Ci.nsIFileProtocolHandler)
	.getURLSpecFromFile)

for (let it in new Iterator(
	{
		localFile: ['@mozilla.org/file/local;1',
   'nsILocalFile'],
		fileInputStream: ['@mozilla.org/network/file-input-stream;1',
   'nsIFileInputStream'],
		transferable: ['@mozilla.org/widget/transferable;1',
   'nsITransferable'],
	})) let ([name, [cid, iid]] = it)
{
	cid = Cc[cid]
...
var clip = UC.clip = {
	...
	get: function clip_get(flavor, context)
	{
		...
		return arguments.length > 1 ? Array.map(arguments, get) : flavor.map ? flavor.map(get) : get(flavor)
	},
...
function UC_load(win)
{
	...
	{
		href
	} = win.location, done = this.done = {}, start = Date.now()
	for (let [path, depth] in new Iterator(UC.paths))
		if (depth > 0)
		{
	...
	function touch(file)
	{
		...
		for each(let rurl in data.requires) done[rurl] =
			...
			'-> ' + meta.name
		ext == 'XUL' ? win.setTimeout(UC_loadXUL, meta.delay || UC.DELAY_XUL, spec, win) : UC['load' + ext](spec + '?' + data.timestamp, win)
		done[spec] = meta
	}
	if (UC_pget('extensions.uc.log.loaded'))
	{
		let list = [m.name || u + '\n' + m
			for ([u, m] in new Iterator(done))]
...
function UC_loadJS(url, ctx)
{
	try
	{
		return Services.scriptloader.loadSubScript(url, ctx.defaultView || ctx, 'utf-8')
	}
	catch (e)
	{
		if (e !== 'stop')
...
function UC_loadCSS(url, ctx)
{
	...
	return doc.contentType == 'application/vnd.mozilla.xul+xml' ? doc.insertBefore(
		doc.createProcessingInstruction(
			'xml-stylesheet',
			'href="' + UC_hescape(url) + '"'),
		doc.documentElement) : doc.body.appendChild(
		UC_dom(
		{
			$: 'link',
			rel: 'stylesheet',
...
function UC_sort(arr, key, dsc)
{
	var pry = (
		...
		key != null ? function pry(x) x[key] : function idt(x) x)
	var sorted = [
		{
			k: pry(x),
			v: x
		}
		for each(x in Array.slice(arr))].sort(
		dsc ? function dsc(a, b) a.k < b.k : function asc(a, b) a.k > b.k)
	...
	if (obj.$)
	{
		...
		for (let k in obj) switch (k)
		{
			case '$':
				break
			case '_':
				lm.appendChild(UC_dom(obj[k], lm));
				break
			default:
				lm.setAttribute(k, obj[k])
		}
		return lm
	}
...
function UC_toString()[u + '\n' + m
		for ([u, m] in new Iterator(this.done || 0))].join('\n\n') ||
	'[object UC]'
...
Meta.prototype.toString = function Meta_toString(tab)
{
	...
	var s = this.name,
		re = /^/mg
	for (let [k, v] in new Iterator(this))
		if (k !== 'name')
			s += '\n@' + k + '\n' + v.replace(re, tab)
	return s
}
